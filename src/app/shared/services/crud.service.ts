...
import {
    addDoc, collection, collectionData, CollectionReference, deleteDoc, doc, docData,
    DocumentData, DocumentReference, Firestore, query, updateDoc, where
} from '@angular/fire/firestore';
import { from, Observable, of } from 'rxjs';
import { map, mergeMap } from 'rxjs/operators';
import { DbEntity } from '../models/db-entity.model';
...
export abstract class CrudService<T extends DbEntity>{
    ...
    public create(doc: T): Observable<DocumentReference<DocumentData>> {
        return from(addDoc(this.collectionRef, { ...doc }))
    }
    ...
    public update(entity: T, id: string): Observable<void> {
        const docRef = doc(this.firestore, this.collectionName, id);
        const newObject: any = { ...entity };
        return from(updateDoc(docRef, newObject));
    }
